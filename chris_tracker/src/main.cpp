...
//        {
// 	std::vector<cv::Point3d> lineSet;
// 	std::vector<cv::Point3d> origins=originPlys[i];
//          for(int j=0; j<origins.size()-1; j++ )
// 	 {
// 	   cv::Point2d p1st(origins[j].x, origins[j].y);
// 	   cv::Point2d p2nd(origins[j+1].x, origins[j+1].y);
// 	   double distance=sqrt((p1st.x-p2nd.x)*(p1st.x-p2nd.x)+(p1st.y-p2nd.y)*(p1st.y-p2nd.y));
// 	   if(distance<threshold)
// 	   {
// 	    lineSet.push_back(cv::Point3d(p1st.x, p1st.y, -1));
...
// 	  }
// 	  double DIndex=0;
// 	  int runtimes=distance/incrementalX;
// 	  for(int m=0;m<runtimes;m++) // scan using x
// 	  {
// 	        cv::Point2d firstP=getPointInLine2(DIndex, p1st, p2nd);
// 	        DIndex+=incrementalX;
// 	        lineSet.push_back(cv::Point3d(firstP.x, firstP.y, -1));
